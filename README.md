...
**Design Principle:** Structural failures often appear when locally correct outputs violate constraints or state established earlier in a workflow.

---

## Relationship to Current LLM Evaluation Practice

Most current evaluation pipelines focus on **single-turn output scoring** — measuring correctness, helpfulness, or safety within an isolated response.

This repository explores **structural evaluation**, where model behavior is tested across transformations, delegation, and multi-step reasoning workflows.

Instead of asking *“Is this output correct?”*, these probes ask:

**“Does the system preserve constraints and state across operations?”**
...
1. Establish invariant and required state  
2. Delegate the task to another agent/module  
3. Evaluate whether the downstream agent preserves and applies the constraint  
